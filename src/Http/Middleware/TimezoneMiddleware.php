...
class TimezoneMiddleware
{
    ...
    public function handle(Request $request, \Closure $next)
    {
        ...
        $session = $request->session()->has('srylius_user_timezone');

        // Check if there is a language preference in the cookie data
        $cookie = $request->cookie('srylius_user_timezone');

        // If authorized, learn the user's currency preference.
        $preference = $request->user() ? $request->user()->timezone : null;
        ...
        if ($session)
            app('config')->set('app.timezone', $request->session()->get('srylius_user_timezone'));

        // If the user is a visitor, use the language selected with the cookie data.
        if ($cookie)
            app('config')->set('app.timezone', $cookie);

        // If there is no preference then set it to the fallback locale.
        if (!$session && !$cookie)
            app('config')->set('app.timezone', config('app.fallback_timezone'));
